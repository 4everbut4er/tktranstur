...
@section('breadcrumbs', Breadcrumbs::render('admin.review.add'))
                    ...
                    <div class="ibox-content">
                        <form method="get" class="form-horizontal" id="add_review">
                            ...
                            <div class="form-group"><label class="col-sm-2 control-label">Компания</label>
                                <div class="col-sm-10"><input type="text" class="form-control" name="company"></div>
                            </div>
                            <div class="hr-line-dashed"></div>
                            ...
                            <div class="form-group">
                                <div class="col-sm-4 col-sm-offset-2">
                                    <button class="btn btn-primary" type="submit">Создать отзыв</button>
                                </div>
                            </div>
                        </form>
                    </div>
...
<script>
    $("#add_review").submit(function( event ) {
        ...
        $.ajax({
            method: "POST",
            url: "{{ route('admin.review.add.json') }}",
            data: data
        })
        .done(function( data ) {
            toastr.success('Отзыв был добавлен');
            setTimeout(function(){
                window.location.replace("{{ route('admin.review') }}")
            }, 800)
        })
        .error(function( msg ) {
            console.log(msg);
